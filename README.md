...
- `box_with_lid.blend` — the first 3D-printing test: a 40 × 40 × 30 mm open-top box (4 mm walls and floor, 32 × 32 × 26 mm cavity) and a separate 4 mm lid whose square plug inserts into the opening to keep it in place — 31.4 mm at the base (0.3 mm/side clearance) tapering to 30.6 mm at the tip for easy entry. Boolean-built watertight solids, modeled in millimeter units in print orientation (no supports needed). Scene `PrintBox`.
- `box_body.stl` / `box_lid.stl` — slicer-ready millimeter STL exports of the two parts, for generating 3D-printer g-code.
